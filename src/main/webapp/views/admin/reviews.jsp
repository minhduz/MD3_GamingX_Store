  ...
  <div class="sidebar pe-4 pb-3">
    <nav class="navbar bg-secondary navbar-dark">
      ...
      <div class="navbar-nav w-100">
        ...
        <a href="<%=request.getContextPath()%>/ReviewServlet" class="nav-item nav-link active"><i class="fa-solid fa-comments"></i>Reviews</a>
      </div>
    </nav>
  </div>
  <!-- Sidebar End -->
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <c:forEach items="${listReview}" var="re">
                  <tr>
                    <td scope="col">${re.reviewID}</td>
                    <td scope="col">${re.userName}</td>
                    <td scope="col">${re.reviewGameName}</td>
                    <td scope="col">${re.reviewContent}</td>
                    <td scope="col">${re.reviewStatus?"Approve":"Unapprove"}</td>
                    <td scope="col">
                      <button type="button" class="btn btn-outline-success m-2" data-bs-toggle="modal" data-bs-target="#updateReviewModal" id="updateReview">Edit</button>
                      <input type="hidden" id="reId" value="${re.reviewID}"/>
                    </td>
                  </tr>
                </c:forEach>
                </tbody>
  ...
  <div class="modal fade" id="updateReviewModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div style="background-color: #191C24" class="modal-content">
        <form action="<%=request.getContextPath()%>/ReviewServlet?action=Update" method="post">
          ...
          <div class="modal-body">
            <table class="table">
              <tr>
                <th scope="col"><label for="updateReviewID" class="form-label">ReviewID</label></th>
                <th scope="col"><input id="updateReviewID" type="text" name="reviewID" class="form-control" readonly></th>
              </tr>
              <tr>
                <th scope="col"><label class="form-label">Review Status</label></th>
                <th>
                  <input id="updateTrue" class="form-check-input updateReviewStatus" type="radio" name="reviewStatus" value="true">
                  <label for="updateTrue" class="form-check-label">Approve</label>
                  <input id="updateFalse" style="margin-left: 20px" class="form-check-input updateReviewStatus" type="radio" name="reviewStatus" value="false">
                  <label for="updateFalse" class="form-check-label">Unapprove</label>
                </th>
              </tr>
            </table>
          </div>
...
<script type="text/javascript">
  $(document).on('click','table #updateReview',function (){
    let reId = $(this).parent().find('#reId').val();
    console.log(reId);
    $.ajax({
      type:'GET',
      url: '${pageContext.request.contextPath}/ReviewServlet?action=GetByID&&reviewID='+reId,
      success:function (reviewUpdate){
        console.log(reviewUpdate);
        $('#updateReviewID').val(reviewUpdate.reviewID);
        let reviewStatus = reviewUpdate.reviewStatus;
        if(reviewStatus){
